...
export function Header({ actions }: { actions?: React.ReactNode }) {
	...
	return (
		...
		>
			<nav
				className={cn(
					'flex h-20 w-full items-center justify-between px-6 md:h-24 md:transition-all md:ease-out',
					{
						'md:px-8': scrolled,
					},
				)}
			>
				...
				<div className="hidden items-center gap-4 md:flex">
					{links.map((link, i) => (
						<a key={i} className={buttonVariants({ variant: 'ghost', size: 'lg' })} href={link.href}>
							{link.label}
						</a>
					))}
				...
				<Button size="icon" variant="outline" onClick={() => setOpen(!open)} className="md:hidden h-12 w-12">
